...
function validateSchemaCompatibility(devSchema, prodSchema) {
  const errors = [];
  const warnings = [];
  ...
  try {
    ...
    errors.push(`Schema parsing error: ${error.message || 'Unknown error'}`);
  }
...
function isTypeIncompatible(devType, prodType) {
...
module.exports = { validateSchemaCompatibility, parseSchema };
